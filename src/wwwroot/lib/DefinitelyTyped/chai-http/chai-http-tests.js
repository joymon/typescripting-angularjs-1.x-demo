...
chai.request(app)
    .put('/user/me')
    .send({ passsword: '123', confirmPassword: '123' })
    .then(function (res) { return chai.expect(res).to.have.status(200); })
...
when(chai.request(app).get('/')).done(function () { return console.log('success'); }, function () { return console.log('failure'); });
