...
myapp.controller("MyController", ["$scope", "$firebase", '$FirebaseObject', '$FirebaseArray',
    function ($scope, $firebase, $FirebaseObject, $FirebaseArray) {
        ...
        // AngularFire
        {
            ...
            sync.$transaction('count', function (currentCount) {
                if (!currentCount)
                    return 1; // Initial value for counter.
                if (currentCount < 0)
        ...
        // AngularFireArray
        {
            ...
            // $watch()
            var unwatch = list.$watch(function (event, key, prevChild) {
                switch (event) {
                    case "child_added":
...
myapp.controller("MyAuthController", ["$scope", "$firebaseAuth",
    function ($scope, $firebaseAuth) {
        ...
        $scope.loginObj.$authWithCustomToken("token").then(function (_) { });
        $scope.loginObj.$authAnonymously().then(function (_) { });
        $scope.loginObj.$authWithPassword(credentials).then(function (_) { });
        $scope.loginObj.$authWithOAuthPopup("github").then(function (_) { });
        $scope.loginObj.$authWithOAuthRedirect("google").then(function (_) { });
        $scope.loginObj.$authWithOAuthToken("twitter", "token").then(function (_) { });
        $scope.loginObj.$getAuth();
        $scope.loginObj.$onAuth(function () { });
        $scope.loginObj.$unauth();
        $scope.loginObj.$waitForAuth();
        $scope.loginObj.$requireAuth();
        $scope.loginObj.$createUser(credentials).then(function (_) { });
        $scope.loginObj.$removeUser(credentials).then(function (_) { });
        $scope.loginObj.$changeEmail(changeUserCredentials).then(function (_) { });
        $scope.loginObj.$changePassword(changePasswordCredentials).then(function (_) { });
        $scope.loginObj.$resetPassword(resetPasswordCredentials).then(function (_) { });
    }
]);
